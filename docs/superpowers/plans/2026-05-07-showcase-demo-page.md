# Showcase Demo Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rebuild `src/App.vue` as a production-quality, single-page scrolling showcase demonstrating all 8 drag-drop use cases in a clean light-theme design — replacing the current basic prototype.

**Architecture:** Single `src/App.vue` file — all 8 demo sections + shared styles. No new components. Each section is fully self-contained with its own reactive state. Existing logic (typed baskets, event log, move/clone) is preserved and polished; 4 new sections (kanban, sortable list, file drop zone, toggle draggable) are added. Demo build uses existing `vite.demo.config.js`.

**Tech Stack:** Vue 3 Composition API (`<script setup>`), native HTML Drag and Drop API via `vue3-drag-drop` `<Drag>`/`<Drop>` components, plain CSS (no framework).
...
| File | Change |
|---|---|
| `src/App.vue` | Full rewrite — header, 8 demo sections, footer, all styles |

---

## Task 1: Page shell — header, section wrappers, footer, base styles

**Files:**
- Modify: `src/App.vue`

Replace the entire current `src/App.vue` with the page shell. This task produces a styled static page with empty section placeholders — no interactive demos yet.

- [ ] **Step 1: Replace src/App.vue with the shell**
...
```bash
cd /Users/tushar/Development/tushar/vue3-drag-drop
npm run dev &
sleep 4
kill %1
```

Expected: no compilation errors. Page renders header, 8 empty section cards, footer.

- [ ] **Step 3: Commit**

```bash
git add src/App.vue
git commit -m "feat: add showcase page shell with header, 8 section cards, footer"
```

---

## Task 2: Section 1 — Kanban Board

**Files:**
- Modify: `src/App.vue`

Add kanban board reactive state and template into Section 1's `<div class="section-body">`.

- [ ] **Step 1: Add kanban state to `<script setup>`**
...
- [ ] **Step 2: Replace Section 1 `<div class="section-body">` content**
...
- [ ] **Step 3: Add kanban styles to `<style>` (before closing tag)**
...
```bash
git add src/App.vue
git commit -m "feat: add kanban board demo section"
```

---

## Task 3: Section 2 — Sortable List

**Files:**
- Modify: `src/App.vue`

- [ ] **Step 1: Add sortable list state to `<script setup>`**
...
- [ ] **Step 2: Replace Section 2 `<div class="section-body">` content**
...
- [ ] **Step 3: Add sortable styles to `<style>`**
...
```bash
git add src/App.vue
git commit -m "feat: add sortable list demo section"
```

---

## Task 4: Section 3 — File Drop Zone

**Files:**
- Modify: `src/App.vue`

Note: File dropping uses `nativeEvent.dataTransfer.files` — NOT `transferData` (which is null for OS files). The `Drop` component's `@drop` handler receives `(transferData, nativeEvent)`.

- [ ] **Step 1: Add file drop state to `<script setup>`**
...
- [ ] **Step 2: Replace Section 3 `<div class="section-body">` content**
...
- [ ] **Step 3: Add file drop styles to `<style>`**
...
```bash
git add src/App.vue
git commit -m "feat: add file drop zone demo section"
```

---

## Task 5: Section 4 — Custom Drag Ghost

**Files:**
- Modify: `src/App.vue`

Show two side-by-side draggable items: one with default browser ghost, one with custom `v-slot:image` ghost.

- [ ] **Step 1: Add ghost demo state to `<script setup>`**
...
- [ ] **Step 2: Replace Section 4 `<div class="section-body">` content**
...
- [ ] **Step 3: Add ghost demo styles to `<style>`**
...
```bash
git add src/App.vue
git commit -m "feat: add custom drag ghost comparison demo section"
```

---

## Task 6: Section 5 — Toggle Draggable

**Files:**
- Modify: `src/App.vue`

- [ ] **Step 1: Add toggle draggable state to `<script setup>`**
...
- [ ] **Step 2: Replace Section 5 `<div class="section-body">` content**
...
- [ ] **Step 3: Add toggle styles to `<style>`**
...
```bash
git add src/App.vue
git commit -m "feat: add toggle draggable demo section"
```

---

## Task 7: Section 6 — Move vs Clone

**Files:**
- Modify: `src/App.vue`

Port the existing move/clone logic into the new section card, removing the old standalone section.

- [ ] **Step 1: Add move/clone state to `<script setup>`**
...
- [ ] **Step 2: Replace Section 6 `<div class="section-body">` content**
...
- [ ] **Step 3: Add move/clone styles to `<style>`**
...
```bash
git add src/App.vue
git commit -m "feat: add move vs clone demo section"
```

---

## Task 8: Section 7 — Event Log

**Files:**
- Modify: `src/App.vue`

Global event log that captures events from ALL draggables on the page.

- [ ] **Step 1: Add event log state to `<script setup>`**
...
- [ ] **Step 2: Wire `logEvent` onto ALL existing Drag/Drop elements**

On every `<Drag>` in the file, add (if not already present):
```html
@dragstart="(d) => logEvent('dragstart', d)"
@dragend="(d) => logEvent('dragend', d)"
```

On every `<Drop>` in the file, add:
```html
@dragenter="(d) => logEvent('dragenter', d)"
@dragleave="(d) => logEvent('dragleave', d)"
```

- [ ] **Step 3: Replace Section 7 `<div class="section-body">` content**
...
- [ ] **Step 4: Add event log styles to `<style>`**
...
```bash
git add src/App.vue
git commit -m "feat: add global event log demo section with timestamps"
```

---

## Task 9: Section 8 — Typed Drop Zones

**Files:**
- Modify: `src/App.vue`

Port and polish the existing fruit/veg/file typed basket logic into Section 8.

- [ ] **Step 1: Add typed zones state to `<script setup>`**
...
- [ ] **Step 2: Replace Section 8 `<div class="section-body">` content**
...
- [ ] **Step 3: Add typed zones styles to `<style>`**
...
- [ ] **Step 4: Verify demo build**

```bash
npm run build:demo 2>&1 | tail -8
```

Expected: clean build, `dist-demo/` generated with no errors.

- [ ] **Step 5: Commit**

```bash
git add src/App.vue
git commit -m "feat: add typed drop zones demo section; complete showcase page"
```
...
|---|---|
| Page shell: header, footer, 8 section cards, base styles | Task 1 |
| Kanban board (3 columns, card drag between) | Task 2 |
| Sortable list (reorder within one list) | Task 3 |
| File drop zone (OS files, image accept, others reject) | Task 4 |
| Custom drag ghost comparison (default vs slot) | Task 5 |
| Toggle draggable (enable/disable per item) | Task 6 |
| Move vs clone between lists | Task 7 |
| Global event log with timestamps, color-coded event names | Task 8 |
| Typed drop zones (fruit/veg/file, conditional accept) | Task 9 |
| Demo build (`npm run build:demo`) passes | Task 9 Step 4 |

No placeholders. All function names, ref names, and CSS class names consistent across tasks.
